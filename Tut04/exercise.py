...
save_dir = 'checkpoints-ex/'

if not os.path.exists(save_dir):
    os.makedirs(save_dir)

save_path = os.path.join(save_dir, 'best-val')
...
def optimize(num_iterations):
    ...
    for i in range(num_iterations):
        ...
        if (total_iterations % 100 == 0) or (i == num_iterations - 1):
            ...
            saver.save(sess=session, save_path=ckpt_path, global_step=int(total_iterations/100))
...
# RESTORE LATEST VARIABLES #
saver.restore(sess=session, save_path=tf.train.latest_checkpoint(save_dir))
# helper.plot_conv_weights(session, weights=weights_conv_1)
print_test_accuracy(show_example_errors=True)
